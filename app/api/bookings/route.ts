...
// Business hours check (Dominican Republic - AST, UTC-4)
function isAfterBusinessHours(): boolean {
  const now = new Date();
  const utcHours = now.getUTCHours();
  const drHour = ((utcHours - 4) + 24) % 24;
  const drDay = now.getUTCDay();
  ...
  switch (adjustedDay) {
    case 0: return drHour < 10 || drHour >= 16;
    case 6: return drHour < 9 || drHour >= 18;
    default: return drHour < 8 || drHour >= 20;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Honeypot check - if 'companyName' field is filled, it's a bot
    if (body.companyName) {
      console.log('Spam detected: booking honeypot field filled');
    ...
    // Create a lead if booking is received after business hours
    try {
      const afterHours = isAfterBusinessHours();
      if (afterHours) {
        const followUpDate = getNextBusinessDay();
        await supabaseDb
          .from('leads')
          .insert({
            name: `${body.customerFirstName} ${body.customerLastName}`,
            email: body.customerEmail,
            phone: body.customerPhone || null,
            subject: `Booking: ${service.title_en || 'Service'}`,
            message: `New booking (${bookingReference}) for ${body.numPassengers} passengers on ${dateStr} at ${body.serviceTime}. Received after hours.`,
            source: 'booking',
